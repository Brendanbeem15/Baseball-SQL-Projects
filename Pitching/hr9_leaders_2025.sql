/*
Question:
Which MLB pitchers with at least 30 innings pitched allowed the fewest HR per 9 during the 2025 season?
...
SELECT pt.yearid, p.nameFirst, p.nameLast,   (SUM(pt.IPouts) / 3) || '.' || (SUM(pt.IPouts) % 3) AS IP, SUM(pt.HR) AS HR,
 ROUND(
    CAST(
        (
            SUM(pt.HR)*9
