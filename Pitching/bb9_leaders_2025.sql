/*
Question:
Which MLB pitchers with at least 30 innings pitched had the lowest BB per 9 during the 2025 season?
...
SELECT pt.yearid, p.nameFirst, p.nameLast,  SUM(pt.BB) AS BB, (SUM(pt.IPouts) / 3) || '.' || (SUM(pt.IPouts) % 3) AS IP,
 ROUND(
    CAST(
        (
            SUM(pt.BB)*9
